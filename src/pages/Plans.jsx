...
const plans = [
  {
    ...
    price: "$11.99",
    account: 1,
    feature: ["Ad-free", "Download music", "High Fidelity Sound", "Unlimited and on demand"],
  },
  {
    name: "FAMILY",
    price: "$19.99",
    ...
    <>
      ...
      >
        <div className="flex w-full justify-between gap-10 items-center">
          <h1 className={`${name == "INDIVIDUAL" ? "text-primary" : "text-[#FF673D]"} text-xl xl:text-3xl lg:text-2xl text-secondary text-center font-black`}>{name}</h1>
          <p className="text-sm  text-gray-700 text-center">{account} account</p>
        </div>
  ...
  return (
    <>
      ...
      <div className="w-full bg-white h-dvh flex flex-col justify-center">
        ...
        >
          <div>
            <div className="text-neutral-800 text-3xl md:text-6xl lg:text-7xl font-black py-4 w-full">Get 1 month of Premium</div>
            <div className="text-primary text-3xl md:text-6xl lg:text-7xl font-black py-4">For Free</div>
            <div className="flex flex-col gap-6 w-auto">
              <p className="text-neutral-500 w-auto">We will remind you 7 days before your trial ends.</p>
              <Button variant={"primary"} type={"clickable"} title={"Subscirbe"}></Button>
            </div>
          </div>
          <div className="flex flex-col gap-8 xl:gap-10">
            <div className="text-white lg:text-xl font-normal xl:max-w-xl">
              Innovative Software House to Bring Your Ideas to Life. We are your trusted partner in web, mobile, and software development.
            </div>
          </div>
        </motion.div>
      </div>
      ...
      >
        ...
        <section className="flex gap-6 sm:flex-row w-full justify-center">{plans.map((plan, index) => card(index, plan.name, plan.account, plan.feature, plan.price))}</section>
      </motion.div>
      {/* Services Section */}
